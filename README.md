...
Este proyecto utiliza React para el frontend, Node.js para el backend y MySQL como base de datos. A continuación se detallan los pasos para configurar y arrancar el proyecto.
...
3. Iniciar la aplicación de React:
    ...
    ```bash
    cd appVerificacionCrisViteReact
    npm run dev
    ```

Si se ha iniciado correctamente, debería estar corriendo en `http://localhost:5173`.
...
```sql
    ...
    CREATE DATABASE producto_verificacion;
```
2. Crear las tablas necesarias ejecutando las siguientes consultas SQL en la base de datos anteriormente creada:
...
Finalmente, comprobar si todas las tablas y los datos de las mismas se han creado correctamente.